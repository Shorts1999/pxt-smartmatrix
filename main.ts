...
namespace SmartMatrix {
    ...
    export function getLettermap(char: string): number[] {
        ...
        if (offset >= 0) {
            for (let i = 0; i < 8; i++) {
                letterMap[i] = font.getNumber(NumberFormat.UInt8BE, (offset * 8 + i)); //Every character has 8 arguments in the array, so multiply the offset by 8, and then take ne next 8 arguments as the value for the correct bitmap.
                //letterMap[i] = Test[i]
            }
        }
        return letterMap;
