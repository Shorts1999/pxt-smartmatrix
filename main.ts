...
04043C4444443C00 0000384478403800 1820207820202000 00003C44443C038
...
let Test = [255, 0, 255, 0, 255, 0, 255, 0];
...
namespace SmartMatrix {
    ...
    export class Matrix {
        ...
        drawBitmap(bitmap: number[], x: number, y: number, width: number, heigth: number, colour: number, angle: number = 0): void {
    ...
    export function getLettermap(char: string): number[] {
        ...
        if (offset >= 0) {
            for (let i = 0; i < 8; i++) {
                letterMap[i] = 5
                //letterMap[i] = font.getNumber(NumberFormat.UInt8BE, i); //Every character has 8 arguments in the array, so multiply the offset by 8, and then take ne next 8 arguments as the value for the correct bitmap.
                serial.writeNumber(letterMap[i])
